...
public class MVCPortletChangeXML  extends XMLFileMigrator {
	...
	protected List<SearchResult> searchXMLFile(File file) {
		...
		try {
			results.addAll(xmlFileChecker.findTag("portlet-class", "com.liferay.util.bridges.mvc.MVCPortlet"));
			results.addAll(xmlFileChecker.findTag("name", "copy-request-parameters"));

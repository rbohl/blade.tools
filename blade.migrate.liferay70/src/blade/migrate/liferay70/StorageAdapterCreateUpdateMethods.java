...
@Component(
	property = {
		...
		"problem.title=StorageAdapter API Changes", "problem.type=java",
...
public class StorageAdapterCreateUpdateMethods extends JavaFileMigrator {
	...
	protected List<SearchResult> searchJavaFile(File file) {
		...
		List<SearchResult> invocations = javaFileChecker.findMethodInvocations(null, "StorageEngineUtil", "create",
				new String[] { "long", "long", "Fields" ,"ServiceContext"});

		searchResults.addAll(invocations);

		invocations = javaFileChecker.findMethodInvocations(null, "StorageEngineUtil", "update",
